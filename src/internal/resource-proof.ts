...
function reject(
  reason: "invalid_proof" | "proof_limit_exceeded",
  message: string
): never {
...
class JsonParser {
  ...
  parse(): unknown {
    ...
    if (this.index !== this.source.length)
      reject("invalid_proof", "Unexpected data after proof JSON");
    return value;
  }

  private value(depth: number): unknown {
    if (depth > MAX_DEPTH)
      reject("proof_limit_exceeded", "Proof JSON exceeds its depth limit");
    this.space();
  ...
  private object(depth: number): Record<string, unknown> {
    ...
    while (true) {
      this.space();
      if (this.source[this.index] !== '"')
        reject("invalid_proof", "Proof object key must be a string");
      const key = this.string();
      if (keys.has(key))
        reject(
          "invalid_proof",
          "Proof object contains a duplicate decoded key"
        );
      keys.add(key);
      this.space();
      if (this.source[this.index] !== ":")
        reject("invalid_proof", "Proof object is missing a colon");
      this.index += 1;
      this.countElement();
      ...
      if (separator !== ",")
        reject("invalid_proof", "Proof object is not terminated");
    }
  }
  ...
  private array(depth: number): ReadonlyArray<unknown> {
    ...
    while (true) {
      ...
      if (separator === "]") return output;
      if (separator !== ",")
        reject("invalid_proof", "Proof array is not terminated");
    }
  }
  ...
  private string(): string {
    ...
    while (this.index < this.source.length) {
      ...
      if (code < 0x20)
        reject("invalid_proof", "Proof string contains a control character");
      if (code === 0x22 && !escaped) {
        ...
        try {
          ...
          reject("invalid_proof", "Proof string is malformed");
        }
        for (let offset = 0; offset < decoded.length; offset += 1) {
          ...
          if (unit >= 0xd800 && unit <= 0xdbff) {
            ...
            if (!Number.isInteger(low) || low < 0xdc00 || low > 0xdfff)
              reject(
                "invalid_proof",
                "Proof string contains an isolated surrogate"
              );
            offset += 1;
  ...
  private number(): number {
    ...
    if (match === null)
      reject("invalid_proof", "Proof JSON contains an invalid value");
    this.index += match[0].length;
    const value = Number(match[0]);
    if (!Number.isFinite(value))
      reject("invalid_proof", "Proof number is not finite");
    return value;
  }
...
export function parseProofDocument(
  bytes: Uint8Array
): Readonly<CanonicalProofDocument> {
  if (bytes.byteLength > MAX_RAW_BYTES)
    reject("proof_limit_exceeded", "Proof JSON exceeds its raw byte limit");
  let source: string;
  try {
    source = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  } catch {
    reject("invalid_proof", "Proof JSON is not valid UTF-8");
  }
  const value = new JsonParser(source).parse();
  const json = canonicalJson(value);
  const byteCount = Buffer.byteLength(json, "utf8");
  if (byteCount > MAX_CANONICAL_BYTES)
    reject(
      "proof_limit_exceeded",
      "Proof JSON exceeds its canonical byte limit"
    );
...
function normalizedLiteral(input: string): string {
  if (
    ...
    throw new ResourceProofRejectedError(
      "permission_mismatch",
      "Workspace access must use relative literal paths"
    );
  }
  ...
  if (
    ...
    throw new ResourceProofRejectedError(
      "permission_mismatch",
      "Workspace access contains a non-normalized path segment"
    );
  }
  return parts.join("/");
}
...
function normalizeScope(
  scope: Readonly<WorkspaceAccessScope>
): WorkspaceAccessScope {
  if (scope.kind !== "literals") return Object.freeze({ kind: scope.kind });
  if (scope.paths.length === 0) {
    throw new ResourceProofRejectedError(
      "permission_mismatch",
      "A literal workspace scope must not be empty"
    );
  }
  return Object.freeze({
    kind: "literals",
    paths: Object.freeze(
      [...new Set(scope.paths.map(normalizedLiteral))].sort()
    ),
  });
}
...
function resourceKey(resource: Readonly<ExternalResourcePermission>): string {
  if (resource.authorityId.length === 0 || resource.selector.length === 0) {
    throw new ResourceProofRejectedError(
      "permission_mismatch",
      "External resource identifiers must not be empty"
    );
  }
...
export function normalizePermissionManifest(
  input: Readonly<PermissionManifest>
): Readonly<PermissionManifest> {
  ...
  for (const resource of input.externalResources) {
    ...
    if (previous !== undefined && previous.usage !== resource.usage) {
      throw new ResourceProofRejectedError(
        ...
        "One external resource has conflicting usage modes"
      );
    }
  ...
  if (tools.some((tool) => tool.length === 0)) {
    throw new ResourceProofRejectedError(
      "permission_mismatch",
      "Tool identifiers must not be empty"
    );
  }
  return Object.freeze({
    ...
    externalResources: Object.freeze(
      [...resources.values()].sort((left, right) =>
        `${resourceKey(left)}\u0000${left.usage}`.localeCompare(
          `${resourceKey(right)}\u0000${right.usage}`
        )
      )
    ),
  });
...
export function permissionManifestsMatch(
  ...
  return isDeepStrictEqual(
    normalizePermissionManifest(requested),
    normalizePermissionManifest(effective)
  );
}
  ...
  while (true) {
    try {
      ...
      if (code !== "ENOENT") {
        throw new ResourceProofRejectedError(
          "permission_mismatch",
          "Workspace path cannot be inspected"
        );
      }
      ...
      suffix.push(
        candidate.slice(parent.length + (parent.endsWith(sep) ? 0 : 1))
      );
      candidate = parent;
    }
  }
...
export async function validateWorkspaceScope(
  ...
  for (const literal of scope.paths) {
    ...
    const resolvedLiteral = relative(workspaceRoot, target)
      .split(sep)
      .join("/");
    const remainsAllowed = scope.paths.some(
      (allowed) =>
    ...
    if (!remainsAllowed) {
      throw new ResourceProofRejectedError(
        "permission_mismatch",
        "Workspace path resolves outside its declared literal scope"
      );
    }
  }
  return true;
...
export function permissionManifestDocument(
  manifest: Readonly<PermissionManifest>
): Readonly<CanonicalProofDocument> {
  return parseProofDocument(
    Buffer.from(canonicalJson(normalizePermissionManifest(manifest)), "utf8")
  );
}
