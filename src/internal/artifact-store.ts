...
const RESULT_ACCEPTANCE_PREPARATION_SCHEMA =
...
type RegistrationState =
  "receiving" | "prepared" | "registered" | "aborted" | "unresolved";
...
interface PinRecord {
  ...
  readonly ownerType:
    "artifact_use_binding" | "principal" | "result_acceptance_preparation";
...
function resultAcceptanceOutcome(
  ...
  const preparation: ResultAcceptancePreparationSnapshot = {
    ...(record.state === "preparing" || record.state === "unresolved"
      ? {}
      : { evidence: record.evidence }),
    state: record.state,
    ...(record.retentionUntil === undefined
      ? {}
  ...
  if (record.state === "prepared")
    return {
      kind: "prepared",
      preparation: { ...preparation, evidence: record.evidence },
    };
  ...
  return {
    kind: "failed",
    terminal: false,
    reason: record.failureReason ?? "storage_inspection_unavailable",
  };
}
...
function useBindingOutcome(
  record: UseBindingRecord
): ArtifactUseBindingOutcome {
  if (record.state === "available")
    return { kind: "available", binding: record };
  if (record.state === "released") return { kind: "released", binding: record };
  if (record.state === "preparing")
    return { kind: "continuable", binding: record };
  return useBindingFailed(
...
function authorityFailure(
  decision: ArtifactAuthorityDecision
): ArtifactFailureReason | undefined {
  switch (decision) {
    case "allowed":
      return undefined;
    case "denied":
...
function outcome(record: RegistrationRecord): ArtifactRegistrationOutcome {
  ...
  if (record.state === "receiving" || record.state === "prepared") {
    return {
      ...
      registration: registrationSnapshot(record),
    };
  }
...
function policyIsValid(
  value: unknown
): value is OpenArtifactStoreOptions["policy"] {
...
function validatePolicy(policy: OpenArtifactStoreOptions["policy"]): void {
  if (!policyIsValid(policy)) {
    throw new ArtifactStoreOpenError(
      "invalid_policy",
      "Artifact storage limits must be positive safe integers"
    );
  }
}

function storedFormats(
  formats: ReadonlyArray<Readonly<ArtifactFormat>>
): ReadonlyArray<StoredFormat> {
  return formats
    .map(({ formatId, normalizationId, validatorId }) => ({
      formatId,
      normalizationId,
      validatorId,
    }))
    .sort((left, right) =>
      `${left.formatId}\0${left.normalizationId}`.localeCompare(
...
function validRequestShape(
  ...
  return (
    IDENTIFIER.test(request.registrationId) &&
    Number.isSafeInteger(request.expectedByteCount) &&
    request.expectedByteCount >= 0 &&
    /^sha256:[a-f0-9]{64}$/u.test(request.expectedDigest) &&
    IDENTIFIER.test(request.formatId) &&
    IDENTIFIER.test(request.normalizationId) &&
    request.dependencies.every((dependency) => IDENTIFIER.test(dependency)) &&
    Number.isSafeInteger(request.recoveryBudget) &&
    request.recoveryBudget > 0 &&
...
class FileArtifactStore implements ArtifactStore {
  ...
  private readonly activeTransfers = new Set<
  ...
  constructor(
    ...
    this.resultAcceptanceRetentionDirectory = join(
      options.rootDirectory,
      "result-acceptance-retentions"
    );
    this.garbageCollectionDiagnosticDirectory = join(
      options.rootDirectory,
      "gc-diagnostics"
    );
  ...
  async writerOwnership(): Promise<Readonly<ArtifactWriterOwnership>> {
    ...
    if (owner.pid !== process.pid || typeof owner.startToken !== "string") {
      throw new ArtifactStoreOpenError(
        "writer_locked",
        "Artifact writer ownership changed"
      );
    }
  ...
  private async fault(point: ArtifactStoreFaultPoint): Promise<void> {
    try {
      ...
      throw new ArtifactStoreInjectedFault(
        `Injected Artifact storage fault at ${point}`,
        { cause: error }
      );
    }
  }
  ...
  private async serializeIo<Value>(
    operation: () => Promise<Value>
  ): Promise<Value> {
    const previous = this.ioTail;
    let release!: () => void;
    this.ioTail = new Promise<void>((resolve) => {
      release = resolve;
    });
    await previous;
    try {
  ...
  private async readResultAcceptancePreparation(
    ...
    try {
      const value = (await readJson(
        this.resultAcceptancePreparationPath(preparationId)
      )) as Partial<ResultAcceptancePreparationRecord>;
      if (
        value.schema !== RESULT_ACCEPTANCE_PREPARATION_SCHEMA ||
        value.request?.preparationId !== preparationId ||
        value.requestDigest !== resultAcceptanceRequestDigest(value.request) ||
        typeof value.subjectId !== "string" ||
        value.evidence?.preparationId !== preparationId ||
        value.evidence.digest !==
          resultAcceptanceEvidenceDigest({
            ...
            acceptedArtifactRetentionMs:
              value.evidence.acceptedArtifactRetentionMs,
            retentionPolicyDigest: value.evidence.retentionPolicyDigest,
          }) ||
        value.retentionPolicy?.formatId !==
          "pions.result-acceptance-retention-policy.v1" ||
        ...
        value.retentionPolicy.digest !==
          retentionPolicyDigest({
            formatId: value.retentionPolicy.formatId,
            operationId: value.retentionPolicy.operationId,
            acceptedArtifactRetentionMs:
              value.retentionPolicy.acceptedArtifactRetentionMs,
          }) ||
        value.retentionPolicy.acceptedArtifactRetentionMs !==
          value.effectiveAcceptedRetentionMs ||
        ...
        !Number.isSafeInteger(value.effectiveAcceptedRetentionMs) ||
        (value.effectiveAcceptedRetentionMs ?? 0) <= 0
      ) {
  ...
  private async readResultAcceptanceRetention(
    preparationId: string
  ): Promise<ResultAcceptanceRetentionRecord | undefined> {
    try {
      const value = (await readJson(
        this.resultAcceptanceRetentionPath(preparationId)
      )) as Partial<ResultAcceptanceRetentionRecord>;
      if (
        value.schema !== RESULT_ACCEPTANCE_RETENTION_SCHEMA ||
        value.preparationId !== preparationId ||
        !IDENTIFIER.test(value.operationId ?? "") ||
        !IDENTIFIER.test(value.acceptanceRequestId ?? "") ||
        ...
        (value.retentionMs ?? 0) <= 0 ||
        !["pending", "active", "cancelled"].includes(value.state ?? "") ||
        (value.state === "active" &&
          (!Number.isFinite(Date.parse(value.retainUntil ?? "")) ||
            value.eventEvidence?.state !== "accepted" ||
            value.eventEvidence.preparationId !== preparationId))
      ) {
  ...
  private async readExplicitPin(
    pinId: string
  ): Promise<ExplicitPinRecord | undefined> {
    if (!IDENTIFIER.test(pinId)) return undefined;
    try {
      const value = (await readJson(
        this.explicitPinPath(pinId)
      )) as Partial<ExplicitPinRecord>;
      ...
      if (
        value.schema !== EXPLICIT_PIN_SCHEMA ||
        value.pinId !== pinId ||
        !IDENTIFIER.test(request.artifactId) ||
        !IDENTIFIER.test(request.ownerId) ||
        typeof request.purpose !== "string" ||
        request.purpose.length === 0 ||
        request.retention !== "indefinite" ||
        value.requestDigest !== pinRequestDigest(request) ||
        typeof value.subjectId !== "string" ||
        !Array.isArray(value.dependencyClosure) ||
        !value.dependencyClosure.every(
          (id) => typeof id === "string" && IDENTIFIER.test(id)
        ) ||
        (value.state !== "held" && value.state !== "released")
      )
        throw new Error("Invalid explicit Artifact pin");
  ...
  private async readUseBinding(
    bindingId: string
  ): Promise<UseBindingRecord | undefined> {
    if (!IDENTIFIER.test(bindingId)) return undefined;
    try {
      const value = (await readJson(
        this.useBindingPath(bindingId)
      )) as Partial<UseBindingRecord>;
      const states = [
        "preparing",
        "available",
        "rejected",
      ...
      if (
        value.schema !== USE_SCHEMA ||
        !IDENTIFIER.test(bindingId) ||
        value.bindingId !== bindingId ||
        !IDENTIFIER.test(request.operationId) ||
        !IDENTIFIER.test(request.artifactId) ||
        request.purpose !== "review_subject" ||
        !IDENTIFIER.test(request.decisionId) ||
        typeof request.authorityBasis !== "string" ||
        request.authorityBasis.length === 0 ||
        value.requestDigest !== useBindingRequestDigest(request) ||
        typeof value.subjectId !== "string" ||
        !Array.isArray(value.dependencyClosure) ||
        !value.dependencyClosure.every(
          (id) => typeof id === "string" && IDENTIFIER.test(id)
        ...
        !Number.isSafeInteger(value.effectiveReviewRetentionMs) ||
        (value.effectiveReviewRetentionMs ?? 0) <= 0
      ) {
  ...
  private writeUseBinding(record: UseBindingRecord): Promise<void> {
    return this.serializeIo(() =>
      writeJson(this.useBindingPath(record.bindingId), record)
    );
  }
  ...
  private async readRecord(
    ...
    try {
      ...
      if (!status.isFile() || status.isSymbolicLink())
        throw new Error("Invalid Artifact registration path");
      const value = (await readJson(path)) as Partial<RegistrationRecord>;
      const states: ReadonlyArray<RegistrationState> = [
        "receiving",
        "prepared",
        "registered",
        "aborted",
      ...
      if (
        ...
        value.request?.registrationId !== registrationId ||
        !validRequestShape(value.request) ||
        value.requestDigest !== requestDigest(value.request) ||
        typeof value.subjectId !== "string" ||
        value.subjectId.length === 0 ||
        typeof value.artifactId !== "string" ||
        !IDENTIFIER.test(value.artifactId) ||
        !policyIsValid(value.effectivePolicy) ||
        !states.includes(value.state as RegistrationState) ||
        !Number.isSafeInteger(value.transferGeneration) ||
        (value.transferGeneration ?? -1) < 0 ||
        !Number.isSafeInteger(value.recoveryAttempts) ||
        (value.recoveryAttempts ?? -1) < 0 ||
        (value.temporaryFile !== undefined &&
          (!value.temporaryFile.startsWith(
            `${temporaryPrefix(registrationId)}${value.transferGeneration}-`
          ) ||
            !value.temporaryFile.endsWith(".part"))) ||
        (value.state === "prepared" &&
          typeof value.temporaryFile !== "string") ||
        (value.state === "registered" &&
          (value.artifact?.artifactId !== value.artifactId ||
            value.artifact.digest !== value.request.expectedDigest ||
            ...
            JSON.stringify(value.artifact.dependencies) !==
              JSON.stringify(value.request.dependencies)))
      ) {
  ...
  private async writeRecordUnserialized(
    record: RegistrationRecord
  ): Promise<void> {
  ...
  private async abort(
    ...
    const aborted: RegistrationRecord = {
      ...record,
      state: "aborted",
  ...
  private async removeTemporaryFiles(registrationId: string): Promise<void> {
    for (const entry of await readdir(this.registrationDirectory, {
      ...
      if (
        !entry.isFile() ||
        !entry.name.startsWith(temporaryPrefix(registrationId)) ||
  ...
  private async authenticate(
    credential: string
  ): Promise<Readonly<ArtifactPrincipal> | undefined> {
    try {
  ...
  private requestLimitFailure(
    request: Readonly<ArtifactRegistrationRequest>
  ): ArtifactFailureReason | undefined {
    const { policy } = this.options;
    if (
      !validRequestShape(request) ||
      request.expectedByteCount > policy.maxArtifactBytes ||
      request.dependencies.length > policy.maxDirectDependencies ||
      request.recoveryBudget > policy.maxRecoveryAttempts ||
      Date.parse(request.deadline) - this.now().getTime() >
        policy.maxRegistrationWindowMs
    ) {
      return "limit_exceeded";
    }
    if (Date.parse(request.deadline) <= this.now().getTime())
      return "deadline_expired";
    if (!this.formats.has(`${request.formatId}\0${request.normalizationId}`))
  ...
  private async readArtifactRecord(
    ...
    try {
      ...
      const status = await lstat(path);
      if (!status.isFile() || status.isSymbolicLink())
        throw new Error("Invalid Artifact record path");
      ...
      if (
        ...
        (value.lifecycle !== "available" &&
          value.lifecycle !== "deletion_pending" &&
          value.lifecycle !== "deleted") ||
        (value.storageStatus !== "verified" &&
          value.storageStatus !== "corrupt" &&
          value.storageStatus !== "uninspectable") ||
        ...
        !value.dependencies.every(
          (dependency) =>
            typeof dependency === "string" && IDENTIFIER.test(dependency)
        ) ||
        !Number.isFinite(Date.parse(value.registeredAt ?? "")) ||
        !Number.isFinite(Date.parse(value.unusedRetentionUntil ?? "")) ||
        !Number.isSafeInteger(value.deletionRecoveryAttempts) ||
        (value.deletionRecoveryAttempts ?? -1) < 0 ||
        (value.deletionRecoveryBudget !== undefined &&
          (!Number.isSafeInteger(value.deletionRecoveryBudget) ||
            value.deletionRecoveryBudget <= 0))
      ) {
  ...
  private async dependencyClosure(
    ...
    const walk = async (
      ...
      if (
        depth > policy.maxDependencyDepth ||
        visited.size + visiting.size >= policy.maxDependencyCount
      )
        return "limit_exceeded";
      if (authorize !== undefined) {
        let decision: ArtifactAuthorityDecision;
        try {
          decision = await authorize(artifactId);
        } catch {
          return "authority_unavailable";
        }
      ...
      if (record.lifecycle === "deletion_pending")
        return "artifact_deletion_pending";
  ...
  async startRegistration(
    ...
    return this.serialize(() =>
      this.startRegistrationUnserialized(credential, request)
  ...
  private async startRegistrationUnserialized(
    ...
    const existing = IDENTIFIER.test(request.registrationId)
      ? await this.readRecord(request.registrationId)
      : undefined;
    if (existing !== undefined) {
      const authority = await this.registrationAuthority(principal, existing);
      if (authority === "authority_revoked" && existing.state !== "registered")
        return this.abort(existing, authority);
      if (authority !== undefined)
        return failed(authority, authority !== "authority_unavailable");
      if (requestDigest(request) !== existing.requestDigest)
        return failed("request_mismatch");
    ...
    try {
      authority = authorityFailure(await principal.canRegister(request));
    } catch {
    ...
    if (closure.reason !== undefined)
      return failed(closure.reason, closure.reason !== "authority_unavailable");
    ...
    for (const entry of entries) {
      ...
      if (
        ...
        Date.parse(record.request.deadline) <= this.now().getTime()
      ) {
    ...
    if (active >= this.options.policy.maxConcurrentRegistrations)
    ...
    if ((await this.readArtifactRecord(artifactId)) !== undefined)
  ...
  async transfer(
    ...
    if (previousToken !== undefined)
      this.transferCancellations.get(previousToken)?.();
    ...
    const transfer = (async () => {
      try {
        await predecessor;
        return await this.transferUntracked(
          credential,
          registrationId,
          bytes,
          token,
        ...
        if (this.transferTokens.get(registrationId) === token)
          this.transferTokens.delete(registrationId);
        if (this.transferStops.get(registrationId) === stopped)
          this.transferStops.delete(registrationId);
      }
    })();
  ...
  private async transferUntracked(
    ...
    const principal = await this.authenticate(credential);
    if (principal === undefined || !IDENTIFIER.test(registrationId))
    ...
    if (authority !== undefined) {
      if (authority === "authority_revoked" && record.state !== "registered")
        return this.abort(record, authority);
      return failed(authority, authority !== "authority_unavailable");
    }
    ...
    try {
      const source: AsyncIterable<Uint8Array> =
        Symbol.asyncIterator in Object(bytes)
      ...
      while (true) {
        ...
        const next = await new Promise<
          IteratorResult<Uint8Array> | "cancelled" | "expired"
        >((resolveNext, rejectNext) => {
          const timer = setTimeout(() => resolveNext("expired"), remainingMs);
          timer.unref();
          let settled = false;
          const settle = (
            value: IteratorResult<Uint8Array> | "cancelled" | "expired"
          ) => {
            if (settled) return;
            settled = true;
            clearTimeout(timer);
        ...
        if (!(chunk instanceof Uint8Array))
          return this.abort(record, "input_integrity_mismatch");
        count += chunk.byteLength;
        if (
          count > record.request.expectedByteCount ||
          count > record.effectivePolicy.maxArtifactBytes ||
          this.temporaryBytes + chunk.byteLength >
            record.effectivePolicy.maxTemporaryBytes
        ) {
          return this.abort(record, "limit_exceeded");
        }
        this.temporaryBytes += chunk.byteLength;
        this.transferByteCounts.set(
          token,
          (this.transferByteCounts.get(token) ?? 0) + chunk.byteLength
        );
      ...
      if (`sha256:${hash.digest("hex")}` !== record.request.expectedDigest)
        return this.abort(record, "input_integrity_mismatch");
      await handle.sync();
      ...
      const format = this.formats.get(
        `${record.request.formatId}\0${record.request.normalizationId}`
      );
      if (
        format === undefined ||
        !(await format.validate(Uint8Array.from(transferred)))
      ) {
        record = {
          ...record,
          state: "aborted",
      ...
      const currentAuthority = await this.registrationAuthority(
        principal,
        record
      );
      if (currentAuthority !== undefined) {
        if (currentAuthority === "authority_unavailable")
          return failed(currentAuthority, false);
      ...
      if (closure.reason !== undefined) {
        if (closure.reason === "authority_unavailable")
          return failed(closure.reason, false);
      ...
      const latest = await this.readRecord(registrationId).catch(
        () => undefined
      );
      const published = await this.readArtifactRecord(record.artifactId).catch(
        () => undefined
      );
      if (complete || latest?.state !== "prepared" || published !== undefined) {
        await unlink(temporaryPath).catch(() => undefined);
        this.temporaryBytes = Math.max(
          0,
          this.temporaryBytes - (this.transferByteCounts.get(token) ?? 0)
        );
      }
  ...
  private commitAuthorized(
    ...
    return this.serializeIo(async () => {
      ...
      const authority = await this.registrationAuthority(principal, record);
      if (authority !== undefined) {
        if (authority === "authority_unavailable")
          return failed(authority, false);
        const aborted = {
          ...record,
          state: "aborted",
  ...
  private async commitUnserialized(
    ...
    let artifact: ArtifactRecord = {
      ...
      unusedRetentionUntil: new Date(
        registeredAt.getTime() +
          record.effectivePolicy.unusedArtifactRetentionMs
      ).toISOString(),
    ...
    if (await exists(artifactPath)) {
      ...
      if (
        ...
        existing.formatId !== artifact.formatId ||
  ...
  async registrationStatus(
    ...
    try {
      ...
      return authority === undefined
        ? outcome(record)
        : failed(authority, authority !== "authority_unavailable");
    } catch {
      return failed("storage_inspection_unavailable");
    }
  }
  ...
  private async recordStorageStatus(
    ...
    await this.serializeIo(async () => {
      ...
      if (
        current === undefined ||
        current.storageStatus === "corrupt" ||
        current.storageStatus === storageStatus
      )
        return;
      await writeJson(this.artifactPath(record.artifactId), {
        ...current,
        storageStatus,
      });
    });
  ...
  private async verifiedBytes(
    record: ArtifactRecord
  ): Promise<Buffer | ArtifactFailureReason> {
    ...
    try {
      ...
      const reason = hasCode(error, "ENOENT")
        ? "stored_artifact_corrupt"
        : "storage_inspection_unavailable";
      await this.recordStorageStatus(
        record,
        reason === "stored_artifact_corrupt" ? "corrupt" : "uninspectable"
      ).catch(() => undefined);
      return reason;
    }
    if (
  ...
  async retrieve(
    ...
    if (
      closure.reason === "unauthorized" ||
      closure.reason === "authority_revoked" ||
      closure.reason === "authority_unavailable"
    ) {
      return retrievalFailed("unauthorized");
    }
    if (closure.reason !== undefined) {
      return retrievalFailed(
        closure.reason === "dependency_not_found"
          ? "stored_artifact_corrupt"
          : closure.reason
      );
    }
  ...
  async prepareUseBinding(
    ...
    try {
      return await this.serialize(() =>
        this.prepareUseBindingUnserialized(credential, request)
      );
    } catch (error) {
      if (error instanceof ArtifactStoreInjectedFault) throw error;
  ...
  private validUseRequest(
    request: Readonly<ArtifactUseBindingRequest>
  ): boolean {
    return (
      IDENTIFIER.test(request.bindingId) &&
      IDENTIFIER.test(request.operationId) &&
      IDENTIFIER.test(request.artifactId) &&
      request.purpose === "review_subject" &&
      IDENTIFIER.test(request.decisionId) &&
  ...
  private async useBindingAuthority(
    ...
    for (const artifactId of artifactIds) {
      try {
        const reason = authorityFailure(
          await principal.canBindArtifactUse(request, artifactId)
        );
        if (reason !== undefined) return reason;
      } catch {
        return "authority_unavailable";
  ...
  private async prepareUseBindingUnserialized(
    ...
    if (this.closed || !this.validUseRequest(request))
      return useBindingFailed("limit_exceeded");
    const principal = await this.authenticate(credential);
    ...
    if (existing !== undefined) {
      if (existing.subjectId !== principal.subjectId)
        return useBindingFailed("unauthorized");
      if (existing.requestDigest !== useBindingRequestDigest(request))
        return useBindingFailed("request_mismatch");
      if (existing.state === "preparing")
        return this.finishPreparingUseBinding(existing, principal);
      return useBindingOutcome(existing);
    }
    const parentAuthority = await this.useBindingAuthority(principal, request, [
      request.artifactId,
    ]);
    if (parentAuthority !== undefined)
      return useBindingFailed(
        parentAuthority,
        parentAuthority !== "authority_unavailable"
      );
    const parent = await this.readArtifactRecord(request.artifactId);
    if (parent === undefined) return useBindingFailed("unauthorized");
    if (parent.lifecycle === "deletion_pending")
      return useBindingFailed("artifact_deletion_pending");
    if (parent.lifecycle === "deleted")
      return useBindingFailed("artifact_deleted");
    const closure = await this.dependencyClosure(
      parent.dependencies,
      this.options.policy,
    ...
    const artifactIds = [
      parent.artifactId,
      ...closure.records.map(({ artifactId }) => artifactId),
    ];
    const retentionUntil = new Date(
      this.now().getTime() + this.options.policy.reviewInputRetentionMs
    ).toISOString();
    ...
    for (const artifactId of artifactIds) {
      ...
      await this.serializeIo(() =>
        writeJson(this.retentionPath(retention.retentionId), retention)
      );
    }
  ...
  private async finishPreparingUseBinding(
    ...
    for (const artifactId of artifactIds) {
      ...
      if (artifact === undefined || artifact.lifecycle !== "available") {
        return this.rejectUseBinding(
          record,
          artifact?.lifecycle === "deletion_pending"
      ...
      if (typeof bytes === "string") {
        if (bytes === "storage_inspection_unavailable")
          return useBindingFailed(bytes, false);
        return this.rejectUseBinding(record, bytes);
      }
    }
    const authority = await this.useBindingAuthority(
      principal,
      request,
      artifactIds
    );
    if (authority !== undefined) {
      if (authority === "authority_unavailable")
        return useBindingFailed(authority, false);
      return this.rejectUseBinding(record, authority);
    }
    const retentionUntil = new Date(
      this.now().getTime() + record.effectiveReviewRetentionMs
    ).toISOString();
    for (const artifactId of artifactIds) {
      const retention: RetentionRecord = {
        schema: RETENTION_SCHEMA,
    ...
    const available: UseBindingRecord = {
      ...record,
      retentionUntil,
      state: "available",
    };
  ...
  private async rejectUseBinding(
    record: UseBindingRecord,
    reason: ArtifactFailureReason
  ): Promise<ArtifactUseBindingOutcome> {
    const rejected: UseBindingRecord = {
      ...record,
      state: "rejected",
      failureReason: reason,
    };
  ...
  private async releaseUseBindingPins(bindingId: string): Promise<void> {
    ...
    for (const entry of await readdir(this.pinDirectory)) {
      ...
      if (
        pin.schema !== PIN_SCHEMA ||
        pin.ownerId !== bindingId ||
        pin.state !== "held"
      )
        continue;
      await this.serializeIo(() =>
        writeJson(path, { ...pin, state: "released" })
      );
    }
  }

  async useBindingStatus(
    credential: string,
    bindingId: string
  ): Promise<ArtifactUseBindingOutcome> {
    try {
      ...
      if (record === undefined || record.subjectId !== principal.subjectId)
        return useBindingFailed("unauthorized");
  ...
  async retrieveForUseBinding(
    ...
    if (status.kind !== "available") {
      return retrievalFailed(
        status.kind === "failed" ? status.reason : "conflict",
        status.kind === "failed" && status.terminal
      );
    }
    ...
    try {
      return await this.serialize(async () => {
        if (
          ...
          return pinFailed("limit_exceeded");
        const principal = await this.authenticate(credential);
        if (principal === undefined) return pinFailed("unauthorized");
        const existing = await this.readExplicitPin(request.pinId);
        if (existing !== undefined) {
          if (existing.subjectId !== principal.subjectId)
            return pinFailed("unauthorized");
          if (existing.requestDigest !== pinRequestDigest(request))
            return pinFailed("request_mismatch");
          return pinOutcome(existing);
        }
        let parentAuthority: ArtifactFailureReason | undefined;
        try {
          parentAuthority = authorityFailure(
            await principal.canPinArtifact(request, request.artifactId)
          );
        } catch {
          parentAuthority = "authority_unavailable";
        }
        ...
        if (parent.lifecycle === "deletion_pending")
          return pinFailed("artifact_deletion_pending");
        if (parent.lifecycle === "deleted")
          return pinFailed("artifact_deleted");
        const closure = await this.dependencyClosure(
          parent.dependencies,
          this.options.policy,
        ...
        const record: ExplicitPinRecord = {
          ...
          subjectId: principal.subjectId,
          dependencyClosure: closure.records.map(
            ({ artifactId }) => artifactId
          ),
          state: "held",
        };
        await this.serializeIo(() =>
          writeJson(this.explicitPinPath(request.pinId), record)
        );
        for (const artifactId of artifactIds) {
          const pin: PinRecord = {
            schema: PIN_SCHEMA,
  ...
  async releaseRetentionPin(
    ...
    try {
      return await this.serialize(async () => {
        ...
        await this.serializeIo(() =>
          writeJson(this.explicitPinPath(pinId), released)
        );
  ...
  async prepareResultAcceptance(
    ...
    try {
      return await this.serialize(() =>
        this.prepareResultAcceptanceUnserialized(credential, request)
      );
    } catch (error) {
      if (error instanceof ArtifactStoreInjectedFault) throw error;
      if (error instanceof ResultAcceptanceManifestError)
        return resultAcceptanceFailed(error.reason);
      return resultAcceptanceFailed("storage_inspection_unavailable", false);
    }
  }

  private validResultAcceptanceRequest(
    request: Readonly<ResultAcceptancePreparationRequest>
  ): boolean {
    return (
      ...
      /^sha256:[a-f0-9]{64}$/u.test(request.manifestDigest) &&
  ...
  private async resultAcceptanceAuthority(
    ...
    for (const artifactId of artifactIds) {
      try {
        const reason = authorityFailure(
          await principal.canPrepareResultAcceptance(request, artifactId)
        );
        if (reason !== undefined) return reason;
      } catch {
        return "authority_unavailable";
  ...
  private async resultAcceptanceArtifacts(
    ...
    ReadonlyArray<ArtifactRecord> | ResultAcceptancePreparationOutcome
  > {
    const roots = [
      request.manifest.bodyArtifactId,
      ...request.manifest.workProducts.flatMap(
    ...
    if (closure.reason !== undefined) {
      const mapped =
        closure.reason === "dependency_not_found"
          ? "artifact_not_found"
          : closure.reason;
      return resultAcceptanceFailed(
        mapped as
          ArtifactFailureReason | ResultAcceptanceManifestError["reason"],
        mapped !== "authority_unavailable"
      );
    }
    ...
    for (const artifactId of roots) {
      const record = await this.readArtifactRecord(artifactId);
      if (record === undefined)
        return resultAcceptanceFailed("artifact_not_found");
      if (record.lifecycle === "deletion_pending")
        return resultAcceptanceFailed("artifact_deletion_pending");
      if (record.lifecycle === "deleted")
        return resultAcceptanceFailed("artifact_deleted");
      records.set(record.artifactId, record);
    }
    ...
    for (const record of ordered) {
      ...
      if (typeof bytes === "string")
        return resultAcceptanceFailed(
          bytes,
          bytes !== "storage_inspection_unavailable"
        );
      const format = this.formats.get(
        `${record.formatId}\0${record.normalizationId}`
      );
      if (format === undefined || !(await format.validate(bytes)))
        return resultAcceptanceFailed("invalid_format");
    }
    return ordered;
  }
  ...
  private async prepareResultAcceptanceUnserialized(
    ...
    if (this.closed || !this.validResultAcceptanceRequest(request))
      return resultAcceptanceFailed("limit_exceeded");
    const principal = await this.authenticate(credential);
    if (principal === undefined) return resultAcceptanceFailed("unauthorized");
    const existing = await this.readResultAcceptancePreparation(
      request.preparationId
    );
    if (existing !== undefined) {
      if (existing.subjectId !== principal.subjectId)
        return resultAcceptanceFailed("unauthorized");
      if (existing.requestDigest !== resultAcceptanceRequestDigest(request))
      ...
      const currentAuthority = await this.resultAcceptanceAuthority(
        principal,
        existing.request,
        existing.evidence.artifactIds
      );
      if (currentAuthority !== undefined) {
        return resultAcceptanceFailed(
          currentAuthority,
          currentAuthority !== "authority_unavailable"
        );
      }
      if (existing.state === "prepared") {
        await this.ensureResultAcceptanceProtections(existing);
        await this.validateResultAcceptanceProtections(existing);
        const artifacts = await this.resultAcceptanceArtifacts(
          existing.request,
          principal
        );
        if ("kind" in artifacts) return artifacts;
        const authority = await this.resultAcceptanceAuthority(
          principal,
          existing.request,
          existing.evidence.artifactIds
        );
    ...
    if (document.digest !== request.manifestDigest)
      return resultAcceptanceFailed("request_mismatch");
    let requirements: Readonly<ResolvedWorkProductRequirements> | "unknown";
    try {
      requirements =
        (await this.options.resultAcceptanceRequirementsSource?.read(
          request.operationId
        )) ?? "unknown";
    } catch {
    ...
    if (requirements === "unknown")
      return resultAcceptanceFailed("authority_unavailable", false);
    if (requirements.digest !== request.requirementsDigest)
      return resultAcceptanceFailed("conflict");
    for (const entry of await readdir(
      this.resultAcceptancePreparationDirectory
    )) {
      if (!entry.endsWith(".json")) continue;
      const other = await this.readResultAcceptancePreparation(
        entry.slice(0, -5)
      );
      if (other?.request.operationId === request.operationId)
        return resultAcceptanceFailed("conflict");
    }
    for (const entry of await readdir(
      this.resultAcceptanceRetentionDirectory
      ...
      if (!entry.endsWith(".json")) continue;
      const other = await this.readResultAcceptanceRetention(
        entry.slice(0, -5)
      );
      if (other?.operationId === request.operationId)
        return resultAcceptanceFailed("conflict");
    }
    let retentionPolicy:
      Readonly<ResultAcceptanceRetentionPolicyEvidence> | "unknown";
    try {
      retentionPolicy =
        (await this.options.resultAcceptanceRetentionPolicySource?.read(
          request.operationId
        )) ?? "unknown";
    } catch {
    ...
    if (retentionPolicy === "unknown")
      return resultAcceptanceFailed("authority_unavailable", false);
    if (
      retentionPolicy.formatId !==
        "pions.result-acceptance-retention-policy.v1" ||
      retentionPolicy.operationId !== request.operationId ||
      retentionPolicy.digest !== request.retentionPolicyDigest ||
      !Number.isSafeInteger(retentionPolicy.acceptedArtifactRetentionMs) ||
      retentionPolicy.acceptedArtifactRetentionMs <= 0 ||
      retentionPolicy.digest !==
        retentionPolicyDigest({
          formatId: retentionPolicy.formatId,
          operationId: retentionPolicy.operationId,
          acceptedArtifactRetentionMs:
            retentionPolicy.acceptedArtifactRetentionMs,
        })
    )
      return resultAcceptanceFailed("conflict");
    const artifacts = await this.resultAcceptanceArtifacts(request, principal);
    if ("kind" in artifacts) return artifacts;
    const validated = validateResultAcceptanceManifest(
      request.manifest,
      requirements,
      artifacts
    );
    ...
    await this.serializeIo(() =>
      writeJson(
        this.resultAcceptancePreparationPath(request.preparationId),
        record
      )
    );
  ...
  private async ensureResultAcceptanceProtections(
    record: ResultAcceptancePreparationRecord
  ): Promise<void> {
    ...
    const existing = await this.readResultAcceptanceRetention(
      record.request.preparationId
    );
    if (existing === undefined) {
      await this.serializeIo(() =>
        writeJson(
          this.resultAcceptanceRetentionPath(record.request.preparationId),
          retention
        )
      );
    } else if (
      existing.state !== "pending" ||
      existing.retentionMs !== retention.retentionMs ||
      JSON.stringify(existing.artifactIds) !==
        JSON.stringify(retention.artifactIds)
    ) {
      throw new Error(
        "Result acceptance retention conflicts with its preparation"
      );
    }
    await this.fault("result_acceptance_retention_persisted");
  }

  private async validateResultAcceptanceProtections(
    record: ResultAcceptancePreparationRecord
  ): Promise<void> {
    const retention = await this.readResultAcceptanceRetention(
      record.request.preparationId
    );
    if (
      retention === undefined ||
      ...
      JSON.stringify(retention.artifactIds) !==
        JSON.stringify(record.evidence.artifactIds)
    ) {
      throw new Error("Result acceptance pending retention is unavailable");
    }
    for (const artifactId of record.evidence.artifactIds) {
      const pinId = protectionId(
        "result-acceptance-pin",
        record.request.preparationId,
        artifactId
      );
      ...
      if (
        pin.schema !== PIN_SCHEMA ||
        pin.pinId !== pinId ||
        pin.ownerType !== "result_acceptance_preparation" ||
        pin.ownerId !== record.request.preparationId ||
        pin.artifactId !== artifactId ||
        pin.state !== "held"
      ) {
  ...
  private async finishPreparingResultAcceptance(
    ...
    if ("kind" in artifacts) return artifacts;
    const validated = validateResultAcceptanceManifest(
      record.request.manifest,
      record.requirements,
      artifacts
    );
    if (
      validated.digest !== record.evidence.manifestDigest ||
      JSON.stringify(validated.artifactIds) !==
        JSON.stringify(record.evidence.artifactIds) ||
    ...
    const prepared: ResultAcceptancePreparationRecord = {
      ...record,
      state: "prepared",
    };
    await this.serializeIo(() =>
      writeJson(
        this.resultAcceptancePreparationPath(record.request.preparationId),
        prepared
      )
    );
    ...
    try {
      return await this.serialize(async () => {
        ...
        if (principal === undefined)
          return resultAcceptanceFailed("unauthorized");
        const record =
          await this.readResultAcceptancePreparation(preparationId);
        if (record === undefined || record.subjectId !== principal.subjectId)
          return resultAcceptanceFailed("unauthorized");
        const currentAuthority = await this.resultAcceptanceAuthority(
          principal,
  ...
  private validEventEvidence(
    evidence: Readonly<ResultAcceptanceEventEvidence>,
    state: ResultAcceptanceEventEvidence["state"]
  ): boolean {
    return (
      IDENTIFIER.test(evidence.preparationId) &&
      IDENTIFIER.test(evidence.operationId) &&
      IDENTIFIER.test(evidence.acceptanceRequestId) &&
  ...
  private eventEvidenceMatches(
    ...
    return (
      ...
      left.acceptanceRequestId === right.acceptanceRequestId &&
  ...
  private eventEvidenceMatchesPreparation(
    ...
    return (
      record.request.operationId === evidence.operationId &&
  ...
  private async reconciliationAuthority(
    ...
    try {
      const trust =
        (await this.options.resultAcceptanceEventEvidenceVerifier?.verify(
          evidence
        )) ?? "unknown";
      if (trust === "unknown") return "authority_unavailable";
      if (trust === "untrusted") return "conflict";
      return authorityFailure(
        await principal.canReconcileResultAcceptance(
  ...
  async finalizeResultAcceptance(
    ...
    try {
      return await this.serialize(async () => {
        if (!this.validEventEvidence(evidence, "accepted"))
          return resultAcceptanceFailed("conflict");
        const principal = await this.authenticate(credential);
        if (principal === undefined)
          return resultAcceptanceFailed("unauthorized");
        const record = await this.readResultAcceptancePreparation(
          evidence.preparationId
        );
        if (record === undefined || record.subjectId !== principal.subjectId)
          return resultAcceptanceFailed("unauthorized");
        ...
        const authority = await this.reconciliationAuthority(
          principal,
          evidence
        );
        if (authority !== undefined)
        ...
        if (record.state === "accepted") {
          if (
            !this.eventEvidenceMatches(record.eventEvidence, evidence) ||
        ...
        if (
          retention === undefined ||
          retention.operationId !== record.request.operationId ||
          retention.acceptanceRequestId !==
            record.request.acceptanceRequestId ||
          retention.retentionMs !== record.effectiveAcceptedRetentionMs ||
          JSON.stringify(retention.artifactIds) !==
            JSON.stringify(record.evidence.artifactIds)
        ) {
        ...
        if (
          retention.state === "active" &&
          (!this.eventEvidenceMatches(retention.eventEvidence, evidence) ||
            retention.retainUntil !== retentionUntil)
        )
          return resultAcceptanceFailed("conflict");
        if (retention.state === "cancelled")
          return resultAcceptanceFailed("conflict");
        ...
        if (retention.state === "pending") {
          ...
          await this.serializeIo(() =>
            writeJson(
              this.resultAcceptanceRetentionPath(evidence.preparationId),
              active
            )
          );
        ...
        await this.serializeIo(() =>
          writeJson(
            this.resultAcceptancePreparationPath(evidence.preparationId),
            accepted
          )
        );
  ...
  async abortResultAcceptance(
    ...
    try {
      return await this.serialize(async () => {
        if (!this.validEventEvidence(evidence, "not_accepted"))
          return resultAcceptanceFailed("conflict");
        const principal = await this.authenticate(credential);
        if (principal === undefined)
          return resultAcceptanceFailed("unauthorized");
        ...
        const currentRetention = await this.readResultAcceptanceRetention(
          evidence.preparationId
        );
        if (currentRetention?.state === "active")
          return resultAcceptanceFailed("conflict");
        if (record.state === "aborted") {
          if (!this.eventEvidenceMatches(record.eventEvidence, evidence))
            return resultAcceptanceFailed("conflict");
        ...
        const aborted: ResultAcceptancePreparationRecord = {
          ...record,
          state: "aborted",
          eventEvidence: evidence,
        };
  ...
  private async releaseResultAcceptancePins(
    preparationId: string
  ): Promise<void> {
  ...
  async collectGarbage(
    ...
    try {
      return await this.serialize(() =>
        this.collectGarbageUnserialized(credential, request)
      );
    } catch (error) {
      if (error instanceof ArtifactStoreInjectedFault) throw error;
      await this.persistGarbageCollectionDiagnostic(
        request,
  ...
  private async persistGarbageCollectionDiagnostic(
    ...
    await this.serializeIo(() =>
      writeJson(
        join(
          this.garbageCollectionDiagnosticDirectory,
          `${request.collectionId}.json`
        ),
        {
          schema: GC_DIAGNOSTIC_SCHEMA,
  ...
  private async collectGarbageUnserialized(
    ...
    if (
      this.closed ||
      !IDENTIFIER.test(request.collectionId) ||
      !Number.isSafeInteger(request.scanBudget) ||
      request.scanBudget <= 0 ||
      request.scanBudget > this.options.policy.maxGarbageCollectionScan ||
      !Number.isSafeInteger(request.deletionBudget) ||
      request.deletionBudget <= 0 ||
      request.deletionBudget >
        this.options.policy.maxGarbageCollectionDeletes ||
      !Number.isSafeInteger(request.recoveryBudget) ||
      request.recoveryBudget <= 0 ||
      request.recoveryBudget >
        this.options.policy.maxGarbageCollectionRecoveryAttempts ||
      (request.afterArtifactId !== undefined &&
        !IDENTIFIER.test(request.afterArtifactId))
    ) {
    ...
    let authority: ArtifactFailureReason | undefined;
    try {
      authority = authorityFailure(await principal.canGarbageCollect(request));
    } catch {
      authority = "authority_unavailable";
    }
    if (authority !== undefined)
      return gcFailed(authority, authority !== "authority_unavailable");
    const artifactIds = (await readdir(this.artifactDirectory))
      ...
      .filter(
        (artifactId) =>
          request.afterArtifactId === undefined ||
          artifactId > request.afterArtifactId
      )
      .sort();
    ...
    for (let index = 0; index < scanned.length; index += 1) {
      ...
      if (record.lifecycle === "deletion_pending") {
        ...
        if (record.deletionRecoveryAttempts >= budget)
          return gcFailed("gc_processing_unavailable");
        const attempting = {
          ...record,
          deletionRecoveryAttempts: record.deletionRecoveryAttempts + 1,
        };
        await this.serializeIo(() =>
          writeJson(this.artifactPath(record.artifactId), attempting)
        );
        try {
      ...
      if (
        !(await this.isDeletionEligible(
          record.artifactId,
          record.unusedRetentionUntil
        ))
      ) {
        lastProcessedArtifactId = artifactId;
        continue;
      ...
      await this.serializeIo(() =>
        writeJson(this.artifactPath(record.artifactId), pending)
      );
  ...
  private async isDeletionEligible(
    artifactId: string,
    unusedRetentionUntil: string
  ): Promise<boolean> {
    ...
    for (const entry of await readdir(this.registrationDirectory)) {
      ...
      if (
        ...
        (registration.state !== "receiving" &&
          registration.state !== "prepared")
      )
        continue;
      if (
        registration.artifactId === artifactId ||
        registration.request.dependencies.includes(artifactId)
      )
        return false;
      const closure = await this.dependencyClosure(
        registration.request.dependencies,
        registration.effectivePolicy
      );
      if (
        closure.reason !== undefined ||
        closure.records.some((record) => record.artifactId === artifactId)
      )
        return false;
    }
    for (const entry of await readdir(this.useBindingDirectory)) {
      ...
      if (
        binding !== undefined &&
        (binding.state === "preparing" || binding.state === "available") &&
        (binding.artifactId === artifactId ||
          binding.dependencyClosure.includes(artifactId))
      )
        return false;
    }
    for (const entry of await readdir(this.explicitPinDirectory)) {
      ...
      if (
        ...
        pin.state === "held" &&
        (pin.artifactId === artifactId ||
          pin.dependencyClosure.includes(artifactId))
      )
        return false;
    }
    for (const entry of await readdir(
      ...
      if (!entry.endsWith(".json")) continue;
      const preparation = await this.readResultAcceptancePreparation(
        entry.slice(0, -5)
      );
      if (
        preparation === undefined ||
        !preparation.evidence.artifactIds.includes(artifactId) ||
        preparation.state === "aborted"
      )
        continue;
      if (preparation.state !== "accepted") return false;
      const retention = await this.readResultAcceptanceRetention(
        preparation.request.preparationId
      );
      if (
        retention === undefined ||
        retention.state !== "active" ||
        ...
        JSON.stringify(retention.artifactIds) !==
          JSON.stringify(preparation.evidence.artifactIds)
      ) {
        throw new Error("Published Result acceptance retention is unavailable");
      }
      if (Date.parse(retention.retainUntil!) > this.now().getTime())
        return false;
    }
    for (const entry of await readdir(
      this.resultAcceptanceRetentionDirectory
      ...
      if (
        retention === undefined ||
        !retention.artifactIds.includes(artifactId) ||
        retention.state === "cancelled"
      )
        continue;
      if (retention.state === "active") {
        const preparation = await this.readResultAcceptancePreparation(
          retention.preparationId
        );
        if (
          preparation?.state !== "accepted" ||
          preparation.request.operationId !== retention.operationId ||
          preparation.request.acceptanceRequestId !==
            retention.acceptanceRequestId ||
          preparation.retentionUntil !== retention.retainUntil ||
          !this.eventEvidenceMatches(
            preparation.eventEvidence,
            retention.eventEvidence!
          ) ||
          JSON.stringify(preparation.evidence.artifactIds) !==
            JSON.stringify(retention.artifactIds)
        ) {
          throw new Error(
            "Published Result acceptance preparation is unavailable"
          );
        }
      }
      if (
        retention.state === "pending" ||
        Date.parse(retention.retainUntil!) > this.now().getTime()
      )
        return false;
    }
    for (const entry of await readdir(this.pinDirectory)) {
      ...
      const pin = (await readJson(
        join(this.pinDirectory, entry)
      )) as Partial<PinRecord>;
      if (
        pin.schema === PIN_SCHEMA &&
        pin.artifactId === artifactId &&
        pin.state === "held"
    ...
    for (const entry of await readdir(this.retentionDirectory)) {
      ...
      const retention = (await readJson(
        join(this.retentionDirectory, entry)
      )) as Partial<RetentionRecord>;
      if (
        retention.schema === RETENTION_SCHEMA &&
        retention.artifactId === artifactId &&
        Date.parse(retention.retainUntil ?? "") > this.now().getTime()
      )
        return false;
    }
  ...
  private async finishDeletion(record: ArtifactRecord): Promise<void> {
    ...
    try {
      ...
      if (!hasCode(error, "ENOENT")) {
        await this.serializeIo(() =>
          writeJson(this.artifactPath(record.artifactId), {
            ...record,
            storageStatus: "uninspectable",
          })
        );
        throw error;
      }
    ...
    await this.serializeIo(() =>
      writeJson(this.artifactPath(record.artifactId), deleted)
    );
    await this.fault("deletion_committed");
  }

  private async preparationSourcesRemainTrusted(
    record: ResultAcceptancePreparationRecord
  ): Promise<boolean> {
    try {
      const requirements =
      ...
      return (
        ...
        retentionPolicy.acceptedArtifactRetentionMs ===
          record.effectiveAcceptedRetentionMs
      );
    } catch {
      return false;
    }
  ...
  private async recoverUseBindingsPinsAndDeletions(): Promise<void> {
    for (const entry of await readdir(
      ...
      if (record.state === "preparing") {
        try {
          ...
          await this.ensureResultAcceptanceProtections(record);
          const principal = await this.options.authenticator.restore(
            record.subjectId
          );
        ...
        try {
          ...
          const evidence =
            sourcedEvidence === "unknown" ? undefined : sourcedEvidence;
          if (evidence === undefined || evidence.acceptedAt === undefined)
            continue;
          const evidenceTrust =
            (await this.options.resultAcceptanceEventEvidenceVerifier?.verify(
              evidence
          ...
          const retentionUntil = new Date(
            Date.parse(evidence.acceptedAt) +
              record.effectiveAcceptedRetentionMs
          ).toISOString();
          const principal = await this.options.authenticator.restore(
            record.subjectId
          ...
          if (
            ...
            if (retention.state === "pending") {
              await writeJson(
                this.resultAcceptanceRetentionPath(
                  record.request.preparationId
                ),
                active
              );
            ...
            await writeJson(
              this.resultAcceptancePreparationPath(
                record.request.preparationId
              ),
              accepted
            );
            await this.releaseResultAcceptancePins(
              record.request.preparationId
            );
          }
        } catch (error) {
        ...
        try {
          ...
          const evidenceTrust =
            record.eventEvidence === undefined
              ? "unknown"
              : ((await this.options.resultAcceptanceEventEvidenceVerifier?.verify(
          ...
          if (
            ...
            retention !== undefined &&
            retention.state === "active" &&
            retention.eventEvidence !== undefined &&
            record.eventEvidence !== undefined &&
            this.validEventEvidence(record.eventEvidence, "accepted") &&
            this.eventEvidenceMatchesPreparation(
              record,
              record.eventEvidence
            ) &&
            this.eventEvidenceMatches(
        ...
        if (
          evidenceTrust !== "trusted" ||
          !this.validEventEvidence(record.eventEvidence, "not_accepted") ||
    ...
    for (const entry of await readdir(this.explicitPinDirectory)) {
      ...
      if (pin?.state === "released")
        await this.releaseExplicitPinRecords(pin.pinId);
    }
    for (const entry of await readdir(this.useBindingDirectory)) {
      if (!entry.endsWith(".json")) continue;
      ...
      for (const artifactId of artifactIds) {
        ...
        if (!(await exists(this.retentionPath(retention.retentionId))))
          await writeJson(this.retentionPath(retention.retentionId), retention);
        const pin: PinRecord = {
          schema: PIN_SCHEMA,
        ...
        if (!(await exists(this.pinPath(pin.pinId))))
          await writeJson(this.pinPath(pin.pinId), pin);
      }
      try {
        const principal = await this.options.authenticator.restore(
          record.subjectId
  ...
  async recover(): Promise<void> {
    ...
    for (const entry of await readdir(this.registrationDirectory)) {
      ...
      if (record.recoveryAttempts >= record.request.recoveryBudget) {
        ...
        if (
          published !== undefined &&
          typeof (await this.verifiedBytes(published)) !== "string"
        ) {
          ...
          await this.writeRecord({
            ...record,
            state: "unresolved",
            failureReason: "recovery_budget_exceeded",
          });
        }
        continue;
      }
      ...
      try {
        ...
        const committed = await this.commitAuthorized(record, principal);
        if (
          committed.kind === "failed" &&
          committed.reason === "authority_unavailable"
        ) {
...
async function acquireLock(rootDirectory: string): Promise<string> {
  ...
  if (startToken === undefined) {
    throw new ArtifactStoreOpenError(
      "writer_locked",
      "Artifact writer ownership is not inspectable"
    );
  }
  ...
  if (
    ...
    throw new ArtifactStoreOpenError(
      "writer_locked",
      "Artifact storage root already has a writer"
    );
  }
...
async function initializeRoot(
  ...
  if (!(await exists(rootDirectory))) {
    ...
    await mkdir(join(rootDirectory, "result-acceptance-retentions"), {
      mode: 0o700,
    });
  ...
  if (!(await exists(rootFile))) {
    if ((await readdir(rootDirectory)).length !== 0)
      throw new ArtifactStoreOpenError(
        "unsupported_root",
        "Artifact storage root has no supported format marker"
      );
  ...
  const marker = (await readJson(rootFile)) as {
    readonly schema?: unknown;
    readonly formats?: unknown;
  };
  if (
    marker.schema !== ROOT_SCHEMA ||
    JSON.stringify(marker.formats) !== JSON.stringify(formats)
  ) {
    throw new ArtifactStoreOpenError(
      "unsupported_root",
      "Unsupported Artifact storage root format or format registry"
    );
  }
  for (const directory of [
    ...
    if (!status.isDirectory() || status.isSymbolicLink())
      throw new ArtifactStoreOpenError(
        "storage_inspection_unavailable",
        "Artifact storage directory is invalid"
      );
  }
}
...
async function openArtifactStoreInternal(
  options: InternalOpenArtifactStoreOptions
): Promise<ArtifactStore> {
  validatePolicy(options.policy);
  const formats = configuredFormats();
  const normalizedOptions = {
    ...options,
    rootDirectory: await canonicalRoot(options.rootDirectory),
  };
  ...
  try {
    await initializeRoot(
      normalizedOptions.rootDirectory,
