...
type WorkerStartReception =
  | {
      ...
      readonly identity: Readonly<WorkerProcessIdentity>;
    }
...
interface Session {
  ...
  resolveStartInstructionAcceptance(
    resolution: StartInstructionAcceptanceResolution
  ): void;
  ...
  resolveStartInstructionAcknowledgement(
    resolution: StartInstructionAcknowledgementResolution
  ): void;
  ...
  descendants: ReadonlyArray<Readonly<DescendantProcessIdentity>> | undefined;
...
/** Upper bound of a persisted agent error message, in UTF-8 bytes. */
export const MAX_AGENT_ERROR_MESSAGE_BYTES = 4096;

/**
 * Keeps the provider's error text verbatim for the caller to interpret,
 * truncated on a UTF-8 character boundary.
 */
export function boundedAgentErrorMessage(message: string): string {
  const bytes = Buffer.from(message, "utf8");
  if (bytes.byteLength <= MAX_AGENT_ERROR_MESSAGE_BYTES) return message;
  let end = MAX_AGENT_ERROR_MESSAGE_BYTES;
...
function parsePromptPath(promptRef: string): string {
  ...
  throw new Error(
    "Visible worker promptRef must be an absolute path or file URL"
  );
}
...
export class VisibleWorker implements WorkerAdapter {
  ...
  private readonly lastKnownIdentities = new Map<
    string,
    Readonly<WorkerProcessIdentity>
  >();
  private readonly processTerminations = new Map<
    string,
    Promise<WorkerCancellationEvidence | undefined>
  >();
  ...
  constructor(private readonly options: VisibleWorkerOptions) {
    this.capabilityGenerator =
      options.capabilityGenerator ?? defaultCapabilityGenerator;
    this.promptReader = options.promptReader ?? defaultPromptReader;
    this.serverFactory = options.serverFactory ?? createServer;
    this.processControl =
      options.processControl ?? new NodeWorkerProcessControl();
    this.cancellationGraceMs = options.cancellationGraceMs ?? 500;
  ...
  open(operation: Operation): Worker {
    ...
    return makeSingleRunWorker({
      ...
      cancel: (_cancellationEpoch, timeoutMs) =>
        Effect.promise(() =>
  ...
  private runSession(
    ...
    return Effect.gen(this, function* () {
      ...
      const launch = yield* Effect.either(
        Effect.tryPromise({
          try: () =>
            recovering
      ...
      if (launch._tag === "Left") {
        if (recovering) {
          return yield* Effect.promise(() =>
            this.stopRecoveredWorker(operation)
          );
        }
        return {
      ...
      return yield* Effect.gen(this, function* () {
        ...
        const mismatch = configurationMismatch(
          ...
          started.identity.observedConfig
        );
        if (mismatch !== undefined)
          return { state: mismatch } as WorkerRunOutcome;
        if (
          recovering &&
          (operation.workerIdentity?.processInstanceId !==
            started.identity.processInstanceId ||
            operation.workerIdentity.processStartToken !==
              started.identity.processStartToken)
        ) {
          return yield* Effect.promise(() =>
            this.stopAfterUncertainStart(operation)
          );
        }
        ...
        if (recovering) {
          ...
          yield* Effect.tryPromise({
            try: () =>
              this.requestDeliveryGenerationUpdate(
                session!,
                instruction.deliveryGeneration,
                instruction.dispatcherId
              ),
          ...
          const generation = yield* receiveWorkerProtocol(
            session!.deliveryGeneration
          );
        ...
        if (!startAlreadyAccepted) {
          ...
          const beginAcceptance = yield* receiveWorkerProtocol(
            session!.startInstructionAcceptance
          );
          ...
          yield* Effect.tryPromise({
            try: () =>
              this.sendStartAcceptanceObservation(session!, instruction),
            catch: (error) =>
              protocolError(
                error instanceof Error ? error.message : String(error)
              ),
          });
          const acknowledgement = yield* receiveWorkerProtocol(
            session!.startInstructionAcknowledgement
          );
        ...
        if (reception.state === "agent_failed") {
          return {
            state: "agent_failed" as const,
            evidence: reception.evidence,
          };
        }
        if (reception.state !== "result_received") {
          return {
            state:
              reception.state === "cancelled"
                ? "worker_protocol_failed"
                : reception.state,
          } as WorkerRunOutcome;
        }
        ...
        const acknowledged = yield* acknowledgeResultAcceptance(
          ...
          (proof) =>
            Effect.tryPromise({
              try: async () => {
                // The Worker exits after ACK, after which its descendants can
                // no longer be traced from its process tree.
                await this.captureDescendants(session!);
                await this.sendAcknowledgement(session!, proof);
              },
              catch: (error) =>
                error instanceof Error ? error : new Error(String(error)),
            })
        ...
        Effect.catchAll(
          (
            ...
            return recovering || startDeliveryEntered
              ...
              : Effect.succeed<WorkerRunOutcome>({
  ...
  private async startWorker(
    ...
    const configPath = join(directory, "worker.v16.json");
    await mkdir(this.options.socketDirectory, {
      recursive: true,
      mode: DIRECTORY_MODE,
    });
    await chmod(this.options.socketDirectory, DIRECTORY_MODE);
    const socketPath = join(
      this.options.socketDirectory,
      `${operationDirectoryKey(operation.operationId)}.sock`
    );
    const prompt = await this.promptReader.read(operation.task.promptRef);
    cancellation.requireLaunchAllowed();
    if (prompt.byteLength > DEFAULT_MAX_PROMPT_BYTES)
      throw new Error("Prompt exceeds the configured size limit");
    ...
    try {
      ...
      const output = await Effect.runPromise(
        this.options.executor.execute({
          ...
          args: [
            "agent",
            "start",
            `pions-${operationDirectoryKey(operation.operationId).slice(0, 26)}`,
            "--kind",
            "pi",
            "--pane",
            operation.presentation.paneId,
            "--timeout",
            String(this.agentStartTimeoutMs),
            "--",
            "--provider",
            ...
            ...effective.extensions.flatMap((extension) => [
              "--extension",
              extension.path,
            ]),
            "--no-skills",
      ...
      if (output.exitCode !== 0)
        throw new Error(
          output.stderr.trim() || "Unable to launch visible worker"
        );
  ...
  private async recoverWorker(
    ...
    if (config.operationId !== operation.operationId) {
      throw new Error(
        "Recovered Worker configuration belongs to another Operation"
      );
    }
    const previous =
      operation.startInstructionDelivery ?? operation.startDeliveryAuthority;
    if (previous === undefined)
      throw new Error("Recovered Operation has no Start delivery authority");
    await mkdir(this.options.socketDirectory, {
      recursive: true,
      mode: DIRECTORY_MODE,
    });
  ...
  private createSession(
    ...
    const startAcknowledgement =
      deferred<StartInstructionAcknowledgementResolution>();
      ...
      try {
        for (const event of session.protocol.receive(chunk)) {
          if (event.type === "started") {
            ...
            session.resolveStartInstructionAcknowledgement({
              state: "acknowledged",
            });
              ...
              session.resolveReception(
                event.type === "worker_failed"
                  ? {
                      ...
                      evidence: {
                        ...event.evidence,
                        errorMessage: boundedAgentErrorMessage(
                          event.errorMessage
                        ),
                      },
                    }
  ...
  private async cancelSession(
    ...
    if (session?.socket === undefined) {
      ...
      if (
        ...
        (storedIdentity === undefined || observedConfig === undefined)
      ) {
        return undefined;
      }
      const identity: WorkerProcessIdentity = knownIdentity ?? {
        ...storedIdentity!,
        observedConfig: observedConfig!,
      };
      const state = await Effect.runPromise(
        this.processControl.observe(identity)
      );
      if (state === "unverifiable") return undefined;
      if (state === "stopped") {
        this.lastKnownIdentities.delete(operation.operationId);
        return { proof: "worker-stop" };
      }
      const descendants = await Effect.runPromise(
        this.processControl.captureDescendants(identity)
      );
      if (descendants === undefined) return undefined;
      const deadline = Date.now() + timeoutMs;
      const evidence = await this.terminateProcess(identity, timeoutMs);
      if (
        evidence === undefined ||
        !(await this.confirmDescendantsStopped(
          descendants,
    ...
    const cancellationDeadline =
      Date.now() +
      Math.max(0, Math.floor(timeoutMs * CANCELLATION_TIMEOUT_BUDGET_RATIO));
    const remaining = () => Math.max(0, cancellationDeadline - Date.now());
    await this.captureDescendants(session, remaining());
    ...
    if (request === undefined) {
      ...
      if (exitObservation !== undefined) {
        ...
        if (stopped) {
          return (await this.confirmSessionDescendantsStopped(
            session,
            remaining(),
            true
          ))
        ...
        return workerEvidence !== undefined &&
          (await this.confirmSessionDescendantsStopped(
            session,
            remaining(),
            true
          ))
      ...
      this.completeCancellation(session);
      return descendantsStopped ? workerEvidence : undefined;
    }
    try {
    ...
    const acknowledged = await this.waitForCancellationAcknowledgement(
      session,
      Math.min(this.cancellationGraceMs, remaining())
    );
    if (!acknowledged) {
      if (session.identity === undefined) return undefined;
      const evidence =
      ...
      this.completeCancellation(session);
      return descendantsStopped ? evidence : undefined;
    }
    if (session.identity === undefined) {
      this.completeCancellation(session);
      return undefined;
    ...
    const descendantsStopped =
      evidence !== undefined &&
      (await this.confirmSessionDescendantsStopped(session, remaining(), true));
    this.completeCancellation(session);
    return descendantsStopped ? evidence : undefined;
  }

  /**
   * Records the Worker's current descendants so their stop can be confirmed
   * after the Worker itself exits. Returns false when they cannot be traced.
   */
  private async captureDescendants(
    session: Session,
    timeoutMs = this.cancellationGraceMs
  ): Promise<boolean> {
    ...
    const identities = new Map(
      (session.descendants ?? []).map((descendant) => [
        `${descendant.processId}:${descendant.processStartToken}`,
        descendant,
      ])
    );
    for (const descendant of captured) {
      identities.set(
        `${descendant.processId}:${descendant.processStartToken}`,
        descendant
      );
    }
    session.descendants = Object.freeze([...identities.values()]);
    return true;
  }

  private confirmSessionDescendantsStopped(
    session: Session,
    timeoutMs: number,
    terminate: boolean
  ): Promise<boolean> {
    return session.descendants === undefined
      ? Promise.resolve(false)
      : this.confirmDescendantsStopped(
          session.descendants,
          timeoutMs,
          terminate
        );
  ...
  private async confirmDescendantsStopped(
    descendants: ReadonlyArray<Readonly<DescendantProcessIdentity>>,
    timeoutMs: number,
    terminate: boolean
  ): Promise<boolean> {
    ...
    const deadline = Date.now() + timeoutMs;
    const state = await Effect.runPromise(
      this.processControl.waitForDescendantsStop(
        descendants,
        terminate ? Math.min(this.exitObservationGraceMs, timeoutMs) : timeoutMs
      )
    );
    if (state === "stopped") return true;
    const remaining = Math.max(0, deadline - Date.now());
    if (!terminate || state === "unverifiable" || remaining === 0) return false;
    return (
      (await Effect.runPromise(
        this.processControl.terminateDescendants(descendants, remaining)
      )) !== undefined
    );
  }
  ...
  private async stopRecoveredWorker(
    operation: Operation
    ...
    const stopped = await this.terminateProcess(
      identity,
      this.agentStartTimeoutMs
    );
  ...
  private terminateProcess(
    ...
    const termination = Effect.runPromise(
      this.processControl.terminate(identity, timeoutMs)
    );
    this.processTerminations.set(key, termination);
    void termination.finally(() => {
      if (this.processTerminations.get(key) === termination)
        this.processTerminations.delete(key);
    });
    return termination;
  }
  ...
  private async requestDeliveryGenerationUpdate(
    ...
    await writeSocket(
      session.socket,
      session.protocol.updateDeliveryGeneration(
        deliveryGeneration,
        dispatcherId
      )
    );
  ...
  private async sendStartAcceptanceObservation(
    ...
    if (session.socket === undefined) {
      throw protocolError(
        "No Worker connection to acknowledge Start acceptance"
      );
    }
    await writeSocket(
      session.socket,
  ...
  private confirmSuccessfulExit(session: Session): Promise<boolean> {
    ...
    const identity = session.identity;
    session.successfulExitObservation ??= (async () => {
      const deadline = Date.now() + this.successfulExitGraceMs;
      const processState = await Effect.runPromise(
        this.processControl.waitForStop(identity, this.successfulExitGraceMs)
      );
      const stopped =
      ...
      this.closeSession(session, stopped);
      return stopped;
    })();
