...
const CONSTRAINTS = ["tools", "read", "write", "commands", "network", "externalResources"] as const;
...
export interface ResourceEvidenceRepository {
  ...
  write(operationId: string, expectedVersion: number | undefined, record: Omit<InternalRecord, "version">): Promise<Readonly<InternalRecord>>;
}

export class InMemoryResourceEvidenceRepository implements ResourceEvidenceRepository {
  private readonly records = new Map<string, InternalRecord>();

  async read(operationId: string): Promise<Readonly<InternalRecord> | undefined> {
    return this.records.get(operationId);
  }

  async write(operationId: string, expectedVersion: number | undefined, input: Omit<InternalRecord, "version">): Promise<Readonly<InternalRecord>> {
    const current = this.records.get(operationId);
    if (current?.version !== expectedVersion) throw new ResourceProofRejectedError("persistence_failed", "Resource evidence changed concurrently");
    const record = structuredClone({ ...input, version: (expectedVersion ?? 0) + 1 });
...
export interface InternalResourceProofController extends ResourceProofController {
  automaticCleanup(operationId: string): Promise<Readonly<VersionedResourceEvidenceSnapshot>>;
  safetyCleanup(operationId: string): Promise<Readonly<VersionedResourceEvidenceSnapshot>>;
  markCleanupUnresolved(operationId: string): Promise<Readonly<VersionedResourceEvidenceSnapshot>>;
}

export interface ResourceProofControllerOptions {
  readonly registrations: ReadonlyArray<Readonly<ResourceAuthorityRegistration>>;
...
function snapshot(record: Readonly<InternalRecord>): Readonly<VersionedResourceEvidenceSnapshot> {
  return Object.freeze({ version: record.version, evidence: structuredClone(record.snapshot) });
}

function rejected(reason: ResourceProofRejectionReason, message: string): never {
...
function requireDocumentBinding(
  ...
  if (document.json !== evidenceDocument(expected).json) rejected("binding_mismatch", message);
}
...
function completeGuarantees(
  ...
  return guarantees.length === CONSTRAINTS.length && CONSTRAINTS.every((constraint) => {
    const guarantee = guarantees.find((candidate) => candidate.constraint === constraint);
    return guarantee !== undefined &&
      ...
      guarantee.method.length > 0 && guarantee.scope.length > 0 && guarantee.basis.length > 0 &&
      guarantee.result === "satisfied" && Number.isFinite(Date.parse(guarantee.checkedAt)) &&
      validDate(guarantee.validUntil) &&
      (guarantee.validUntil !== undefined || guarantee.generation !== undefined);
  });
}
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_resolve, rejectTimeout) => {
        timeout = setTimeout(
          () => rejectTimeout(new ResourceProofRejectedError("cleanup_unresolved", "Resource cleanup timed out")),
          milliseconds,
        );
        timeout.unref();
      }),
...
function validateProofBinding(
  ...
  if (
    ...
    proof.authorityRegistrationId !== request.requirements.authorityRegistrationId ||
  ...
  ) rejected("binding_mismatch", "Resource proof is bound to a different request");
  if (!proof.noConflict || proof.conflictControlId.length === 0) rejected("resource_conflict", "Resource conflict prevention is not proven");
  if (proof.revocationOwner.length === 0) rejected("enforcement_missing", "Resource revocation responsibility is missing");
  if (!completeGuarantees(proof.observations, proof)) rejected("observation_missing", "Current permission observation is incomplete");
  if (!completeGuarantees(proof.enforcements, proof)) rejected("enforcement_missing", "Runtime permission enforcement is incomplete");
  if (!validDate(proof.validUntil) || proof.validUntil === undefined && proof.generation === undefined) {
    rejected("binding_mismatch", "Resource proof is expired or has no validity binding");
  }
}
...
function validateValidationBinding(
  ...
  if (
    ...
    evidence.workerProcessInstanceId !== record.request.workerProcessInstanceId ||
    evidence.authorityId !== record.request.requirements.authorityId ||
    evidence.authorityRegistrationId !== record.request.requirements.authorityRegistrationId ||
    evidence.authorityGeneration !== record.registrationGeneration ||
    evidence.proofDigest !== current.proof?.digest
  ) rejected("binding_mismatch", "Resource validation is bound to a different acquisition");
}

function requireValidHandoff(evidence: Readonly<ResourceValidationEvidence>): void {
  if (!validDate(evidence.validUntil) || evidence.validUntil === undefined && evidence.generation === undefined) {
    rejected("binding_mismatch", "Resource validation is expired or has no validity binding");
  }
  if (evidence.state === "unknown") rejected("validation_unknown", "Resource validation is unknown");
  if (evidence.state !== "valid") rejected("authority_revoked", "Resource validation is invalid");
  if (!evidence.handoffConfirmed) rejected("handoff_unconfirmed", "Resource handoff is not confirmed");
  if (!evidence.relatedExecutionAccessBlocked) rejected("resource_conflict", "Related execution access remains possible");
}

export function makeResourceProofController(options: Readonly<ResourceProofControllerOptions>): InternalResourceProofController {
  const repository = options.repository ?? new InMemoryResourceEvidenceRepository();
  const registrations = new Map(options.registrations.map((registration) => [registration.authorityId, registration]));
  if (registrations.size !== options.registrations.length) throw new ResourceProofRejectedError("invalid_profile", "Authority identifiers must be unique");
  ...
  const registrationFor = (request: Readonly<ResourcePreparationRequest>) => {
    ...
    if (registration === undefined) rejected("authority_unavailable", "Resource authority is not registered");
    if (
      registration.registrationId !== request.requirements.authorityRegistrationId ||
      registration.generation !== request.requirements.authorityGeneration ||
      registration.normalizationVersion !== request.requirements.normalizationVersion
    ) rejected("binding_mismatch", "Resource authority registration does not match the fixed profile");
    return registration;
  };

  const persistDiagnostic = async (record: Readonly<InternalRecord>, reason: ResourceProofRejectionReason) => {
    try {
  ...
  const prepare = async (request: Readonly<ResourcePreparationRequest>) => {
    if (!isDeepStrictEqual(request.workspace, request.requirements.workspace)) {
      rejected("binding_mismatch", "Workspace differs from the fixed resource profile");
    }
    if (!permissionManifestsMatch(request.requestedManifest, request.effectiveManifest) ||
        !permissionManifestsMatch(request.requestedManifest, request.requirements.permissionManifest)) {
      rejected("permission_mismatch", "Requested, effective, and fixed permission manifests differ");
    }
    await validateWorkspaceScope(request.workspace.normalizedPath, request.requestedManifest.read);
    await validateWorkspaceScope(request.workspace.normalizedPath, request.requestedManifest.write);
    const registration = registrationFor(request);
    const trust = await registration.issuer.isCurrentlyTrusted(registration.generation);
    if (trust === "revoked") rejected("authority_revoked", "Resource authority is revoked");
    if (trust === "unknown") rejected("validation_unknown", "Resource authority trust cannot be inspected");
    ...
    for (const resource of request.requestedManifest.externalResources) {
      if (resource.authorityId !== registration.authorityId) {
        rejected("resource_conflict", "One acquisition cannot span unrelated resource authorities");
      }
      const normalized = await registration.adapter.normalizeSelector(
        resource.selector,
        registration.normalizationVersion,
      );
      if (normalized.namespace.length === 0 || normalized.selector.length === 0 || normalized.conflictScopes.length === 0 ||
          normalized.conflictScopes.some((scope) => scope.length === 0)) {
        rejected("binding_mismatch", "Resource selector normalization is incomplete");
      }
      canonicalResources.push(Object.freeze({
        authorityId: registration.authorityId,
        namespace: normalized.namespace,
        normalizationVersion: registration.normalizationVersion,
        selector: normalized.selector,
        conflictScopes: Object.freeze([...new Set(normalized.conflictScopes)].sort()),
        usage: resource.usage,
      }));
    }
    canonicalResources.sort((left, right) => JSON.stringify(left).localeCompare(JSON.stringify(right)));
    ...
    const acquisitionId = digest(`${request.operationId}\u0000${requestDigest}`).slice("sha256:".length);
    ...
    const recovering = record?.snapshot.state === "acquiring" || record?.snapshot.state === "unresolved";
    if (record !== undefined) {
      if (record.snapshot.requestDigest !== requestDigest) rejected("binding_mismatch", "Operation already has a different resource request");
      ...
      record = await repository.write(request.operationId, undefined, {
        ...
        canonicalResources,
        snapshot: { state: "planned", acquisitionId, requestDigest, validations: [], cleanupAttempts: 0 },
      });
    }
    ...
    try {
      if (recovering) {
        ...
        if (recovered === "unknown") rejected("validation_unknown", "Resource acquisition state is unknown");
        proof = recovered;
      } else {
      ...
      acquisitionReachedAdapter = true;
      validateProofBinding(proof, request, acquisitionId, requestDigest, manifest.digest);
      if (!(await registration.issuer.verify(proof.evidence)) || !(await registration.issuer.verify(proof.workspaceEvidence))) {
      ...
      requireDocumentBinding(workspaceDocument, {
        schemaVersion: 1,
      ...
      requireDocumentBinding(document, {
        schemaVersion: 1,
        kind: "resource-proof",
        ...
        ...(proof.validUntil === undefined ? {} : { validUntil: proof.validUntil }),
        ...(proof.generation === undefined ? {} : { generation: proof.generation }),
      }, "Resource proof bytes do not bind the declared acquisition");
      record = await repository.write(request.operationId, record.version, {
        request: record.request,
        registrationGeneration: record.registrationGeneration,
        canonicalResources: record.canonicalResources,
        snapshot: { ...record.snapshot, state: "held", proof: document, workspaceProof: workspaceDocument },
      });
      return snapshot(record);
    } catch (error) {
      const reason = error instanceof ResourceProofRejectedError ? error.reason : "validation_unknown";
      await persistDiagnostic(record, reason);
      if (acquisitionReachedAdapter) {
        await resourceController.safetyCleanup(request.operationId).catch(() => undefined);
      }
      throw error instanceof ResourceProofRejectedError
        ? error
        : new ResourceProofRejectedError(reason, error instanceof Error ? error.message : String(error));
    }
  };

  const readRecord = async (operationId: string): Promise<Readonly<InternalRecord>> => {
    const record = await repository.read(operationId);
    if (record === undefined) rejected("binding_mismatch", "Operation has no resource evidence");
    return record;
  };
  ...
  const resourceController: InternalResourceProofController = {
    ...
    revalidate: async (operationId) => {
      ...
      if (record.snapshot.state !== "held" && record.snapshot.state !== "releasing" || record.snapshot.proof === undefined) rejected("handoff_unconfirmed", "Resource acquisition is not held");
      const registration = registrationFor(record.request);
      const trust = await registration.issuer.isCurrentlyTrusted(record.registrationGeneration);
      if (trust !== "trusted") rejected(trust === "revoked" ? "authority_revoked" : "validation_unknown", "Resource authority is not currently trusted");
      const manifest = permissionManifestDocument(record.request.requestedManifest);
      const adapterRequest = {
      ...
      if (!(await registration.issuer.verify(validation.evidence))) rejected("invalid_proof", "Validation evidence issuer verification failed");
      const document: Readonly<CanonicalProofDocument> = parseProofDocument(validation.evidence);
      requireDocumentBinding(document, {
        schemaVersion: 1,
        ...
        ...(validation.validUntil === undefined ? {} : { validUntil: validation.validUntil }),
        ...(validation.generation === undefined ? {} : { generation: validation.generation }),
        handoffConfirmed: validation.handoffConfirmed,
        relatedExecutionAccessBlocked: validation.relatedExecutionAccessBlocked,
      ...
      record = await repository.write(operationId, record.version, {
        ...
        snapshot: {
          ...record.snapshot,
          validations: [...record.snapshot.validations, { ...validation, evidence: document }],
        },
      });
    ...
    cleanup: async (operationId, credential) => {
      if (cleanupInProgress.has(operationId)) rejected("cleanup_unresolved", "Resource cleanup is already running");
      ...
      if (credential === safetyCleanupCredential) {
        ...
        if (!["inspect", "revoke", "release"].every((operation) => delegated.includes(operation as "inspect" | "revoke" | "release"))) {
          rejected("authority_unavailable", "Required safety cleanup operations were not delegated");
        }
        ...
        if (record.request.requirements.cleanupPolicy !== "automatic") {
          rejected("authority_unavailable", "Automatic cleanup was not delegated for this Operation");
        }
      } else {
        const authenticator = options.cleanupAuthenticator;
        if (authenticator === undefined) rejected("authority_unavailable", "Resource cleanup authentication is unavailable");
        const principal = await authenticator.authenticate(credential);
        actorId = principal.subjectId;
        if (!(await principal.canCleanup(operationId, ["inspect", "revoke", "release"]))) {
          rejected("authority_unavailable", "Actor lacks resource cleanup authority");
        }
      }
      if (record.snapshot.cleanupAttempts >= record.request.requirements.maxCleanupAttempts) {
        rejected("cleanup_unresolved", "Resource cleanup recovery budget is exhausted");
      }
        const registration = registrationFor(record.request);
        const manifest = permissionManifestDocument(record.request.requestedManifest);
        const adapterRequest = {
        ...
        const cleanupId = digest(`${operationId}\u0000${actorId}\u0000${attempt}`).slice("sha256:".length);
        record = await repository.write(operationId, record.version, {
          request: record.request,
          registrationGeneration: record.registrationGeneration,
          canonicalResources: record.canonicalResources,
        ...
        try {
          ...
          if (error instanceof ResourceProofRejectedError && error.reason === "persistence_failed") throw error;
        ...
        if (blocked !== "blocked") {
          ...
          rejected("cleanup_unresolved", "Resource access blocking is unresolved");
        }
        record = await repository.write(operationId, record.version, {
          request: record.request,
          registrationGeneration: record.registrationGeneration,
        ...
        if (released !== "released") rejected("cleanup_unresolved", "Resource release is unresolved");
        return snapshot(record);
      } catch (error) {
        ...
        throw error instanceof ResourceProofRejectedError
          ? error
          : new ResourceProofRejectedError("cleanup_unresolved", error instanceof Error ? error.message : String(error));
      } finally {
