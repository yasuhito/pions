...
async function workerConfigs(root) {
  ...
  for (const repository of await readdir(root, { withFileTypes: true }).catch(
    ...
    for (const operation of await readdir(runtime, {
      withFileTypes: true,
    }).catch(() => [])) {
      if (operation.isDirectory())
        found.push(join(runtime, operation.name, "worker.v7.json"));
    }
  }
  return found;
}
...
for (let attempt = 0; attempt < 6_000; attempt += 1) {
  const candidate = (await workerConfigs(repositories)).find(
...
const suffix = createHash("sha256")
  .update(config.operationId)
  .digest("hex")
  .slice(0, 16);
const proxyDirectory = join(
  tmpdir(),
  `pions-issue29-${process.getuid?.() ?? "user"}`
);
...
const server = createServer((workerSocket) => {
  ...
  runtimeSocket.on("data", (chunk) => {
    ...
    if (
      !didDisconnect &&
      lines.some((line) => line.includes('"type":"begin"'))
    ) {
      didDisconnect = true;
      setTimeout(() => {
        runtimeSocket.end();
...
await writeFile(
  replacementPath,
  `${JSON.stringify({ ...config, socketPath: proxyPath })}\n`,
  { mode: 0o600 }
);
...
console.log(
  "通信を切断しました。オペレーション状態とHerdrペインを確認してください。"
);
await delay(5_000);
server.close();
await rm(proxyPath, { force: true });
