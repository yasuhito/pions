...
class DeferredStopProcessControl implements WorkerProcessControl {
  ...
  waitForStop(
    _identity: Readonly<WorkerProcessIdentity>,
    _timeoutMilliseconds: number
  ) {
...
function operation(
  ...
  return {
    ...
    task: {
      promptRef: "secret prompt reference",
      profile: "coding",
...
function protocolListenerCount(protocolSession: {
  ...
  return (
    protocolSession.server.listenerCount("connection") +
...
async function acknowledgeBegin(
  ...
  const instruction = (JSON.parse(begin) as { readonly instruction: unknown })
    .instruction;
  const observation = receiveFrame(client);
  send(
    client,
    frame(capability, sequenceNumber, "begin_accepted", { instruction })
  );
  await observation;
  send(
    client,
    frame(capability, sequenceNumber + 1, "begin_ack", { instruction })
  );
}
...
async function sendResultDelivery(
  ...
  send(
    client,
    withOperation(frame(options.capability, 1, "hello", { processInstanceId }))
  );
  send(
    client,
    withOperation(
      frame(options.capability, 2, "started", { piSessionId, observedConfig })
    )
  );
  const receivedBegin = await begin;
  const beginInstruction = (
    JSON.parse(receivedBegin) as { readonly instruction: unknown }
  ).instruction;
  const acceptanceObservation = receiveFrame(client);
  send(
    client,
    withOperation(
      frame(options.capability, 3, "begin_accepted", {
  ...
  send(
    client,
    withOperation(
      frame(options.capability, 4, "begin_ack", {
        instruction: beginInstruction,
      })
    )
  );
...
async function deliver(
  ...
  return {
    ...
    begin,
    client,
    outcome: await workerFixture.outcome,
  };
}
...
test("visible Worker recovery does not redispatch a durably accepted Start instruction", async (context) => {
  ...
  const socketPath = join(
    root,
    `${operationDirectoryKey(recovered.operationId)}.sock`
  );
  ...
  client.write(
    peer.send({
      ...
      ...(generation.generationUpdate!.acceptedInstruction === undefined
        ...
        : {
            acceptedInstruction:
              generation.generationUpdate!.acceptedInstruction,
          }),
    })
  );
  ...
  client.write(
    peer.send({
      type: "failed",
      errorMessage: "finished recovery test",
      ...agentRunEvidence,
    })
  );
  await outcome;
...
test("recovery timeout confirms a stopped Worker instead of waiting forever", async (context) => {
  ...
  const outcome = await Effect.runPromise(
    adapter.recover(recovered).run(workerHooks())
  );
...
test("public visible Runtime composes the production path", async (context) => {
  ...
  context.after(() => rm(root, { recursive: true, force: true }));
  const runtime = makeVisibleRuntime({
    cwd: "/work/project",
    stateDirectory: root,
    profiles: { coding: profilePolicy },
    environment: {},
  });
...
test("a Worker cancelled before begin does not receive begin", async (context) => {
  ...
  const receivedControl = JSON.parse(await control) as {
...
test("visible Worker confirms the exact Pi process stopped after ACK", async (context) => {
  ...
  assert.equal(
    processControl.stopObservations[0]?.processStartToken,
    processStartToken
  );
});
...
test("a rejected begin is followed by confirmed Worker stop", async (context) => {
  ...
  const instruction = (
    JSON.parse(await begin) as { readonly instruction: unknown }
  ).instruction;
  const cancellation = receiveFrame(client);
  send(
    client,
    frame(value.capability, 3, "begin_rejected", {
      instruction,
...
test("disconnect releases Worker protocol listeners", async (context) => {
  ...
  send(
    client,
    frame(workerFixture.capability, 1, "hello", { processInstanceId })
  );
  client.end();
  await workerFixture.outcome;
...
test("duplicate Worker cancellation sends one backend request", async (context) => {
  ...
  assert.equal(
    controls
      .map((control) => JSON.parse(control).type)
      .filter((type) => type === "cancel").length,
    1
  );
});
...
test("a Worker rejects a Result acceptance proof for another Operation", async (context) => {
  ...
  const adapter = new VisibleWorker({
    ...
    extensionEntryPath: "/pions/worker-extension.js",
    capabilityGenerator: {
      nextCapability: () => capabilities.shift() ?? "ef".repeat(32),
    },
    promptReader: {
      read: () => Promise.resolve(Buffer.from("private prompt", "utf8")),
    },
